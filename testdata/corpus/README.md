...
|----------------|--------------|-------------|-------------------|
| authentik      | many         | known-gap   | `${PORT:-9000}` env interpolation inside published port |
...
| listmonk       | ~3           | known-gap   | long-syntax volumes (`type: bind`) under `service.volumes` |
| mailcow        | ~25          | known-gap   | env interpolation in published ports, huge service graph |
...
| sentry         | ~30          | known-gap   | long-syntax volumes, massive service graph |
| supabase       | ~15          | known-gap   | env interpolation in published ports, kong/auth/postgres |
